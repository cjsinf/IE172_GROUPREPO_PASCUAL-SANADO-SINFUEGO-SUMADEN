...
app.layout = html.Div(
    [
        dbc.Card(
            [
                dbc.CardHeader("First Dash App", className='app-header'),
                dbc.CardBody(
                    [
                        html.Div(
                            [
                        ...
                        dbc.Button("Calculate!", id='btn_calculate', className='calc-btn', n_clicks=0),
    ...
    if btncalculate_clicks > 0:
        try:
            ...
            try:
                ...
                return [html.Span("Words are not allowed.", className='error-text')]

            if not float_val.is_integer():
                return [html.Span("Integers only.", className='error-text')]

            valid_num = int(float_val)
            if valid_num < 0:
                return [html.Span("No negative numbers.", className='error-text')]
            ...
            return [html.Span(output_val, className='result-text')]

        except Exception:
            return [html.Span("Unknown input, try again!", className='error-text')]
    else:
        raise PreventUpdate
